namespace EntityFrameworkCore.Extensions.Sequences.Test.Services.Tests
{
  ...
  public class SqlServerTest : ITestService
  {
    private readonly TestOptions options;
    private readonly ILogger<SqlServerTest> logger;
    private readonly ILoggerFactory loggerFactory;

    public SqlServerTest(
      ILogger<SqlServerTest> logger,
    ...
    public string Id => "sqlserver-test";
    public bool IsAsync => false;
    public void Run()
    {
      this.logger.LogInformation($"Running synchronous test '{this.Id}'");
      
      if (!this.options.Databases.TryGetValue("sqlserver-testdb", out var dbOptions))
      {
        throw new InvalidOperationException("No database options found for 'sqlserver-testdb'");
      }

      var builder = new DbContextOptionsBuilder<SequenceContext>()
                    .EnableSensitiveDataLogging()
                    .UseLoggerFactory(this.loggerFactory)
                    .UseSqlServer(dbOptions.ConnectionString, server => server.UseSequences());
      
      using var context = new SequenceContext(builder.Options);
      
      this.Test<byte>(context, 1, 1);
      this.Test<short>(context, 1, 1);
      this.Test<int>(context, 1, 1);
